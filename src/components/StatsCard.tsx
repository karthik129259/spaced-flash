...
const StatsCard: React.FC<StatsCardProps> = ({
  ...
  // Add simulated real-time fluctuations for demo purposes
  const [liveValue, setLiveValue] = useState<number>(0);

  // Animate the value counting up with realistic fluctuations
  useEffect(() => {
    if (typeof value === 'number' && animate) {
      ...
      const fluctuationInterval = setInterval(() => {
        ...
        const fluctuation = Math.random() > 0.7 ? Math.random() * 0.05 * numericValue : 0;
        const direction = Math.random() > 0.5 ? 1 : -1;
        const newValue = Math.max(0, numericValue + (direction * fluctuation));
  ...
  return (
    <motion.div
      ...
      className="transform-gpu card-stack"
      whileTap={{ scale: 0.98 }}
    >
      <Card className={cn(
        ...
        {/* Enhanced animated gradient background effect */}        <motion.div 
          className={cn(
          ...
          animate={{
            ...
            opacity: [0.35, 0.45, 0.35], // Subtle breathing effect
          }}
          transition={{
            backgroundPosition: {
              duration: 8,
